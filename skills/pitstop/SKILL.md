...
- **Mis-geocoded stations** — a few registry coordinates are wrong, so `--near` can return a far-away station at a tiny distance. Check the `comune`/`address` of each result.
